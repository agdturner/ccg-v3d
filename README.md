...
## Description
A modularised Java three-dimensional ([3D](https://en.wikipedia.org/wiki/Euclidean_space)) [Euclidean geometry](https://en.wikipedia.org/wiki/Euclidean_geometry) library. The dimensions are defined by orthogonal coordinate axes X, Y and Z that meet at the origin point <x,y,z> where the [Cartesian](https://en.wikipedia.org/wiki/Cartesian_coordinate_system) coordinates x=y=z=0. Coordinates are currently stored as [Math_BigRational](https://github.com/agdturner/ccg-math/blob/master/src/main/java/uk/ac/leeds/ccg/math/number/Math_BigRational.java) numbers comprise of a numerator and a denominator that are Java BigDecimal values. Math_BigRational provides a lot of rational numbers and enables useful exact arithmetic. Conceptually, the cartesian is right-handed. The choice of handedness is explained in the [note on handedness](#handedness). You may be interested in a consideration to do with stipulating the [origin](#origin) and the [geograhical footnote](#geographical-footnote).
...
The reason for right-handedness in physics is to do with rules of thumb which are use to figure out the direction of forces caused by the spin we are in (the Earth, Sun and Milky Way all spin a particular way). [Handedness and chirality](https://en.wikipedia.org/wiki/Chirality_(physics)) and the choice of handedness in physics is explained in [this video](https://youtu.be/BoHQtXpWG2Y). In geography it is common for the X Axis to represent [longitude](https://en.wikipedia.org/wiki/Longitude), the Y Axis to represent [latitude](https://en.wikipedia.org/wiki/Latitude), and the Z axis to represent height above [sea-level](https://en.wikipedia.org/wiki/Sea_level) or depth below sea-level. The choice of the [meridian](https://en.wikipedia.org/wiki/Meridian_(geography)) is arbitrary, values increase to the [East](https://en.wikipedia.org/wiki/East) of the meridian and sometimes also decrease to the [West](https://en.wikipedia.org/wiki/West)) and the Y Axis to represent latitude (that increases from the [equator](https://en.wikipedia.org/wiki/Equator) to the [North](https://en.wikipedia.org/wiki/North) and decreases from the [equator](https://en.wikipedia.org/wiki/Equator) to the [South](https://en.wikipedia.org/wiki/South)). The equator is more discernable than the meridian. Sea-level varies and though it might conceptually be better to fix z=0 at Earth's centre, again this is not necessarily straightforward to pin point!

## [origin](https://en.wikipedia.org/wiki/Origin_(mathematics))
For a lot of physics, it might be sensible to set z=0 at the [centre of mass](https://en.wikipedia.org/wiki/Center_of_mass) (CoM). The thing is, there is typically a bigger and a smaller system to model, and for these there can be a different CoM and a desire to shift the origin and reorientate the axes. If this is the way forward, then some overall framework is wanted at the largest scale to position each entity relative to that. What happens on Earth is influenced significantly by the Sun and Moon and the orbit around the sun is influenced by the orbits and masses of other solar system objects. At another scale, there could be significant effects caused by the position of the [solar system](https://en.wikipedia.org/wiki/Solar_System) in the [galactic plane](https://en.wikipedia.org/wiki/Galactic_plane).
