# [ccga-vector3d](https://github.com/agdturner/ccg-vector3d)
...
The reason for right-handedness in physics is to do with rules of thumb which are use to figure out the direction of forces caused by the spin we are in (the Earth, Sun and Milky Way all spin a particular way). [Handedness and chirality](https://en.wikipedia.org/wiki/Chirality_(physics)) and the choice of handedness in physics is explained in [this video](https://youtu.be/BoHQtXpWG2Y). In geography it is common for the X Axis to represent longitude (that increases from the [meridian](https://en.wikipedia.org/wiki/Meridian_(geography)) to the East and decreases from the merridean to the West) and the Y Axis to represent latitude (that increases from the [equator](https://en.wikipedia.org/wiki/Equator) to the North and decreases from the equator to the South), with the Z Axis increasing from ocean sea-level, and decreasing with ocean depth. Ocean sea-level varies and it might be better to fix z=0 at Earth's centre, but again this is not necessarily straightforward to pin point, and it would be a very geocentric universe if this were to happen!

## The [origin](https://en.wikipedia.org/wiki/Origin_(mathematics))
